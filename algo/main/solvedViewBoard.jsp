...
if((String)session.getAttribute("isProfessor") == "true" || (String)session.getAttribute("isAdmin") == "true"){
	...
	if (order.equals("") || order.equals("deadline-asc"))	order_sql = "order by deadline is null, deadline asc, questionId desc";
	...
	if(search != null && category != null){
		if(category.equals("writer")){
			sql = "select * from question where displayNone=\"false\" and writer like ? " + order_sql + " limit ?,?";
			allDataSql = "select count(*) from question where  displayNone!=\"false\" and writer like ?";
			pstmt = conn.prepareStatement(sql);
			...
			sql = "select * from question where displayNone=\"false\" and subject like ? " + order_sql + " limit ?,?";
			allDataSql = "select count(*) from question where displayNone=\"false\" and subject like ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, "%"+search+"%");
			pstmt.setInt(2, (pageNum-1)*contentView);
			pstmt.setInt(3, contentView);
			...
			sql = "select * from question where displayNone=\"false\" " + order_sql + " limit ?,?";
			allDataSql = "select count(*) from question where displayNone=\"false\"";
			pstmt = conn.prepareStatement(sql);
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
				  ...
				  <%
					...
					out.print("<td style=\"width:100px\">" + order_front + "<b>출제자<b>" + order_tail + "</td>");
				  %>
				  <%
				  ...
				  <%
					...
					out.print("<td style=\"width:90px\"><b>성공자 수/제출자 수<b></td>");
				  %>
				  </tr>
				  <%
					while(rs.next()){
						
						sql = "select count(distinct userId) as allCnt,(select  count(distinct userId) from userLog where questionId=? and status=\"Complete\") as cCnt from userLog where questionId=?";
						pstmt = conn.prepareStatement(sql);
						pstmt.setString(1,rs.getString("questionId"));
						pstmt.setString(2,rs.getString("questionId"));
						ResultSet cnt = pstmt.executeQuery();
						
						cnt.next();
						
						int allCnt = cnt.getInt("allCnt");
						int completeCnt = cnt.getInt("cCnt");
				  ...
				  <td><%=rs.getString("questionId")%></td>
                    ...
                    <td class="mailbox-subject"><a href="./solvedView.jsp?prob=<%=rs.getString("questionId")%>"><%=rs.getString("subject")%></a></td>
                    <td><%=deadline%></td>
                    <td class="mailbox-date"><%=completeCnt%>/<%=allCnt%></td>
                  </tr>
					<% } %>
                  </tbody>
                </table>
