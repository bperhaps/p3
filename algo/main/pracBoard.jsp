...
<%
	...
	String groupSql = "";
		...
		if(groupNum != null){
			ArrayList<Integer> group = groupManager.getCgroup(Integer.parseInt(groupNum));
			groupSql = "groupNum in ("+groupNum+",";
			for(int i=0; i<group.size(); i++){
				groupSql += group.get(i)+",";
			}
	...
	if (order == null) order = "";
	if (order.equals(""))				order_sql = "order by me, questionId desc";
	...
	String tableName = "from question";
	if(groupNum != null && groupSql!="")
		tableName = "from question natural join probGroup";
		
	
	if(search != null && category != null){
		if(category.equals("writer")){
			sql = "select distinct questionId, writer, subject,context,probHash,displayNone,level,startdate,deadline,time,userId,contestId,case when deadline>now() then 1 when deadline<now() then 3 else 2 end as me "+tableName+" where "+groupSql+" displayNone=\"false\" and contestId is null and writer like ? " + order_sql + " limit ?,?";
			allDataSql = "select distinct count(questionId) "+tableName+" where "+groupSql+" displayNone!=\"false\" and writer like ?";
			pstmt = conn.prepareStatement(sql);
			...
			sql = "select distinct questionId, writer, subject,context,probHash,displayNone,level,startdate,deadline,time,userId,contestId,case when deadline>now() then 1 when deadline<now() then 3 else 2 end as me "+tableName+" where "+groupSql+" displayNone=\"false\" and contestId is null and subject like ? " + order_sql + " limit ?,?";
			allDataSql = "select distinct count(questionId) "+tableName+" where "+groupSql+" displayNone=\"false\" and subject like ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, "%"+search+"%");
			pstmt.setInt(2, (pageNum-1)*contentView);
			pstmt.setInt(3, contentView);
			...
			sql = "select distinct questionId, writer, subject,context,probHash,displayNone,level,startdate,deadline,time,userId,contestId,case when deadline>now() then 1 when deadline<now() then 3 else 2 end as me "+tableName+" where "+groupSql+" displayNone=\"false\" and contestId is null " + order_sql + " limit ?,?";
			allDataSql = "select distinct count(questionId) "+tableName+" where "+groupSql+" displayNone=\"false\"";
			pstmt = conn.prepareStatement(sql);
                ...
                <div class="has-feedback">
					<form class="form-search">
						...
						<button type="button" class="btn btn-default selected_category" data-toggle="dropdown">제목
							...
							<ul class="dropdown-menu" style="right:94px">
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
				  ...
				  <%
					...
					String orderForm_front = "<a class=\"order-href\" href=\"" + url + "%s\">";
					...
					if (order.equals("questionId-asc")) {
						...
						String v = urlHandler.getParameter(qs, "order=questionId-asc");
						order_front = String.format(orderForm_front, v);
						order_tail = "</a>";
					}
				  ...
				  <%
					if (order.equals("writer-asc")) {
						...
						String v = urlHandler.getParameter(qs, "order=writer-asc");
						order_front = String.format(orderForm_front, v);
						order_tail = "</a>";
					}
				  ...
				  <%
					if (order.equals("subject-asc")) {
						...
						String v = urlHandler.getParameter(qs, "order=subject-asc");
						order_front = String.format(orderForm_front, v);
						order_tail = "</a>";
					}
				  ...
				  <%
					if (order.equals("deadline-asc")) {
						...
						String v = urlHandler.getParameter(qs, "order=deadline-asc");
						order_front = String.format(orderForm_front, v);
						order_tail = "</a>";
					}
				  ...
				  <%
					if (order.equals("level-asc")) {
						...
						String v = urlHandler.getParameter(qs, "order=level-asc");
						order_front = String.format(orderForm_front, v);
						order_tail = "</a>";
					}
