...
if((String)session.getAttribute("isProfessor") == "true" || (String)session.getAttribute("isAdmin") == "true"){
    ...
    sql = "update question set writer=?, subject=?, context=?,probHash=?,displayNone=?,level=?,startdate=?,deadline=?,time=?,userId=? where questionId=?";
	...
	sql = "delete from probGroup where questionId=?";
	pstmt = conn.prepareStatement(sql);
	pstmt.setInt(1, questionId);
	pstmt.executeUpdate();	
	...
	if(groupNum.length()!=0){
		...
		groupArray.add(groupNum);
	
		String pgroupNum=groupNum;
	
		while(true){	
			sql = "select * from groups where groupNum=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, pgroupNum);
			rs = pstmt.executeQuery();
		
			if(rs.next()&& rs == null)
				return;
		
			if(rs.getInt("parentsGroup") == 0)
				break;
			else{
				groupArray.add(Integer.toString(rs.getInt("parentsGroup")));
				pgroupNum=Integer.toString(rs.getInt("parentsGroup"));
			}
		}
		Iterator<String> itr = groupArray.iterator();
	
		while(itr.hasNext()){
			sql = "insert into probGroup values (?, ?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, questionId);
			pstmt.setString(2, itr.next());
			pstmt.executeUpdate();
		}
	}
	DBUtil.close(conn);
